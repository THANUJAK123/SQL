...
select avg(creditlimit),postalcode from customers
group by postalcode;
select * from orders;
select min(creditlimit) as second_lowest from customers
where creditlimit>(select min(creditlimit) from customers);
select max(creditlimit) from customers;
select * from payments
where year(paymentdate)=2005;
select count(officecode),officecode from offices
...
select quantityInstock,productline from products
group by productline;
select count(ordernumber) from products
where year(orderdate)=2004
(select count(ordernumber) from products where orderdate between 5 and 8);
select count(ordernumber) from orders 
where orderdate  between '2004-05-01' and '2004-08-30';
...
select p%=(('msrp'-'buyproce')/'buyprice')*100 from products;
select count(*),productname from products;
select productname,buyprice,msrp, ((msrp-buyprice)/buyprice)*100 as percentage from products
where ((msrp-buyprice)/buyprice)*100>100;
select count(customername) from customers 
join orders on customers.customernumber=orders.customernumber
join orderdetails on orders.ordernumber=orderdetails.ordernumber
join products on orderdetails.productcode=products.productcode
